...
from src.menu.models import Item
...
def draw_menu(context, menu) -> dict:
    """
    Создает контекстный словарь для передачи в шаблон 'draw_menu.html'.

    - Если пользователь активирует определенный пункт меню, то в словарь
    добавляются все его подпункты.
    """
...
def get_expanded_items_id_list(parent) -> list:
    """
...
def get_child_items(item_values, current_parent_id, expanded_items_id_list) -> list:
    """
